...
- **Property page, first viewport:** gallery → place line → title → asking price → spec strip; the aside holds attribution, one primary CTA plus one secondary (Call the agent, or Message on Instagram), and contact lines with copy email. Then the editorial introduction (lede and body, sans), features, film, financials, location, enquiry, and the **Listing information** disclosure (source, mandate note, Instagram dates).
