...
- **IBM Plex Mono** 400/500 for figures and particulars: meta lines, spec labels, the FX line, counts.
- Scale: display `clamp(2.25rem, …, 4.25rem)` (hero `…3.3rem`), h2 `clamp(1.65rem, …, 2.6rem)`, h3 1.2–1.4rem, body 1.0625rem/1.6, meta .8125rem mono.
...
- Rules, not boxes: sections are separated by 1px `--line` top rules. Only two surfaces are filled: the enquiry aside and the homepage dossier panel.
- Cards are **unboxed**: image, mono place line, title, price and a mono spec row.
...
- Home sections use a different layout family each: split hero, 7/5 curated asymmetric, statement plus three facts, overlapping dossier (image 9 cols, panel 6 cols over it), four-column location register, 6/5 image/text band, ink closing band.
- Property page: mosaic gallery (2:1 lead plus two, "All N photographs") on desktop and a scroll-snap swipe with counter on mobile; then a 7/4 particulars and sticky aside; then ruled 3/8 detail rows; the Photographs row runs full width.

## Components

- **Buttons**: primary (brick), ink, line. 48px minimum height, never wrap. One label per intent: "Enquire" (site contact), "Explore properties", "Arrange a viewing" (for-sale) or "Check availability" (unconfirmed), "Call {first name}" when a phone is published.
